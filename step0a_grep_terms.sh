...
grep -v "^#" Axiom_PMDA.na36.r7.a8.annot.csv | awk -F"," '{print $5,$6,$7,$14,$15,$1,$2,$3}' OFS="\t" | tr -d '"' | awk -F"\t" '{if ($4=="-" && $2!=$3) {$3=$2;} print $0}' OFS="\t" | awk -F"\t" '{if ($5 ~ "//") {split($5,a," "); $4="-"; $5=a[1]; print $0; $5=a[3]; print $0} else {print $0}}' OFS="\t" > Axiom_snps.avinput 
